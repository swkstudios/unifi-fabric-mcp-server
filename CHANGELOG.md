...
- update_setting now detects silent no-op writes and returns a clear
  error when the controller rejects a payload (#17)
- update_network strips read-only fields from input, preventing the
  HTTP 400 that occurred during get-modify-put workflows (#17)
