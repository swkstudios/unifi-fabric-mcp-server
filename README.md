...
An MCP (Model Context Protocol) server that exposes the UniFi Site Manager API as tools for AI assistants. Built with [FastMCP](https://github.com/jlowin/fastmcp), it lets Claude Code, Cline, and other MCP clients manage UniFi network infrastructure through natural language.
...
```mermaid
flowchart LR
    A[AI Assistant<br/>MCP Client] -->|MCP protocol<br/>stdio| B[UniFi Fabric<br/>MCP Server]
...
Generate an API key from [UniFi Account](https://account.ui.com) and keep it handy for the steps below.
...
See [`config/company-mcp.example.json`](config/company-mcp.example.json) for a full example.
...
| `UNIFI_CACHE_MAX_HOSTS` | No | `512` | Max entries in the hosts/ea-sites TTLCache (bounds memory use) |
...
```bash
# SSE transport
docker run -e FASTMCP_TRANSPORT=sse ghcr.io/swkstudios/unifi-fabric-mcp-server

# Stdio transport
docker run -e FASTMCP_TRANSPORT=stdio ghcr.io/swkstudios/unifi-fabric-mcp-server
```
...
**Why stateless?** The server acts as an ephemeral proxy/bridge between AI assistants and the UniFi cloud API. Each heartbeat/session is independent; all configuration and data live in the cloud. Deployment is simplified by container orchestrators (Docker Compose, Kubernetes) with no persistent volume claims needed.
