...
from unifi_fabric.tools.vpn import (
    _delete_radius_profile as delete_radius_profile,
)
from unifi_fabric.tools.vpn import (
...
from unifi_fabric.tools.vpn import (
    _update_radius_profile as update_radius_profile,
)
from unifi_fabric.tools.vpn import (
...
class TestGetVpnServer:
    async def test_basic(self, client, registry):
        ...
        expected_url = f"{_PROXY_BASE}/sites/{SITE_ID}/vpn/servers/vpn-1"
        client.get.assert_called_once_with(expected_url)
        assert result["id"] == "vpn-1"

    async def test_no_data_wrapper(self, client, registry):
        client.get.return_value = {"id": "vpn-1"}
        result = await get_vpn_server(client, registry, "h", "s", "vpn-1")
        assert result["id"] == "vpn-1"
...
class TestGetRadiusProfile:
    async def test_basic(self, client):
        ...
        result = await get_radius_profile(client, "rad-1")
        client.get.assert_called_once_with("/ea/radius-profiles/rad-1")
...
class TestUpdateRadiusProfile:
    async def test_basic(self, client):
        client.patch.return_value = {"data": {"id": "rad-1", "name": "Updated"}}
        result = await update_radius_profile(client, "rad-1", name="Updated")
        client.patch.assert_called_once_with("/ea/radius-profiles/rad-1", json={"name": "Updated"})
        assert result["name"] == "Updated"


class TestDeleteRadiusProfile:
    async def test_basic(self, client):
        client.delete.return_value = None
        result = await delete_radius_profile(client, "rad-1")
        client.delete.assert_called_once_with("/ea/radius-profiles/rad-1")
        assert result == {"deleted": True, "profileId": "rad-1"}
