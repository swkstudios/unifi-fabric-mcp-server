...
async def _get_vpn_server(
    ...
    site_id = await registry.resolve_site_id(site, host_id)
    _assert_uuid(site_id)
    data = await client.get(_proxy(host_id, f"/sites/{site_id}/vpn/servers/{server_id}"))
...
async def _get_radius_profile(
    ...
    data = await client.get(f"/ea/radius-profiles/{profile_id}")
    return cast(dict[str, Any], data.get("data", data))
...
async def _update_radius_profile(
    client: UniFiClient,
    profile_id: str,
    **fields: Any,
) -> dict[str, Any]:
    """Update a RADIUS profile by ID."""
    validate_id(profile_id, "profile_id")
    data = await client.patch(f"/ea/radius-profiles/{profile_id}", json=fields)
    return cast(dict[str, Any], data.get("data", data))


async def _delete_radius_profile(
    client: UniFiClient,
    profile_id: str,
) -> dict[str, Any]:
    """Delete a RADIUS profile by ID."""
    validate_id(profile_id, "profile_id")
    await client.delete(f"/ea/radius-profiles/{profile_id}")
...
def register(mcp: FastMCP, deps_fn: Callable[..., Any]) -> None:
    ...
    async def get_radius_profile(
        ...
        client, _ = deps_fn()
        return await _get_radius_profile(client, profile_id)
    ...
    async def update_radius_profile(
        ...
        fields: fields to update (name, authServerIp, authServerPort, etc.).
        """
        client, _ = deps_fn()
        return await _update_radius_profile(client, profile_id, **fields)

    @mcp.tool()
    async def delete_radius_profile(
        profile_id: str,
    ) -> str:
        """Delete a RADIUS profile by ID. WARNING: Permanently removes RADIUS profile.
        Any SSID or VPN using this profile for 802.1X auth will stop authenticating."""
        client, _ = deps_fn()
        await _delete_radius_profile(client, profile_id)
