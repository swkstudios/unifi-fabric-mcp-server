...
class UniFiClient:
    ...
    async def request(
        ...
        *,
        ...
        """Make an HTTP request with exponential backoff + jitter on 429s."""
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                raise UniFiConnectionError(f"Request timed out: {method} {path} — {exc}") from exc
            except httpx.ConnectError as exc:
                raise UniFiConnectionError(f"Connection failed: {method} {path} — {exc}") from exc
            except httpx.NetworkError as exc:
                raise UniFiConnectionError(f"Network error: {method} {path} — {exc}") from exc

            if resp.status_code != 429:
                if resp.is_error:
                    body = resp.text[:500]
                    raise httpx.HTTPStatusError(
                        f"HTTP {resp.status_code} from {method} {path}: {body}",
